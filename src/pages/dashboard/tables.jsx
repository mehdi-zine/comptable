...
export function Tables() {
  ...
  const [state, setState] = useState({ nom: "", adresse: "", id: "", telephone: "", email: "", budget: 0, numeraire: "" });
  ...
  const saveData = async () => {
    ...
    try {
      ...
      const newBudget =
      type === "Vente"
        ? editedState.budget + numericMontant
        : editedState.budget - numericMontant;

      const updatedState = { ...editedState, budget: newBudget };
      setEditedState(updatedState);
      await setDoc(doc(db, "client", location.state.id), updatedState);
  ...
  const editData = async () => {
    const numericEditedMontant = parseFloat(editedMontant);
    const numericMontant = parseFloat(montant);

    // Reverse the effect of the old transaction on the budget
    let oldImpact = 0;
    if (type === "Vente") {
      oldImpact = numericMontant; // Old Vente added to the budget
    } else if (type === "Achat") {
      oldImpact = -numericMontant; // Old Achat subtracted from the budget
    }

    // Apply the effect of the new transaction
    let newImpact = 0;
    if (editedType === "Vente") {
      newImpact = numericEditedMontant; // New Vente adds to the budget
    } else if (editedType === "Achat") {
      newImpact = -numericEditedMontant; // New Achat subtracts from the budget
    }

  // Calculate the new budget
  const newBudget = editedState.budget - oldImpact + newImpact;
  ...
  const deleteTransaction = async (id, montant, type) => {
    const numericMontant = parseFloat(montant);
    const newBudget =
      type === "Vente"
        ? editedState.budget - numericMontant
      ...
      <Card className="mx-[300px] min-h-[300px]">
        <CardBody>
          ...
          <div className="h-full flex flex-col justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="text-base text-gray-500 w-1/6"> Numeraire </div>
              <div className="w-5/6">
              {isEditing ? (
                <>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    handleChange({ target: { name: "numeraire", value } }); // Update state
                  }}
                  value={editedState.numeraire}
                  className="w-full border border-gray-300 rounded-lg px-2 py-1"
                  placeholder="0.00"
                />
              </>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="w-5/6"> {editedState.budget}€</div>
            </div>
          </div>
        </CardBody>
      </Card>
                    ...
                    <tr key={description}>
                      ...
                      <td className={className}>
                        <div className="flex gap-4">
                          ...
                          <button onClick={() => deleteTransaction(id,montant,type)}
                            className="text-xs font-semibold text-blue-gray-600">
